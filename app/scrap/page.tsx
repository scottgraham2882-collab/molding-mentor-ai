...
type Field = {
  key: keyof Omit<ScrapEntry, "id" | "updatedAt">;
  label: string;
...
const storageKey = "molding-mentor-scrap-entries";

const createEmptyEntry = (): Omit<ScrapEntry, "id" | "updatedAt"> => ({
...
const fields: Field[] = [
  ...
  { key: "partNumber", label: "Part number", placeholder: "PN-88421" },
  { key: "defectType", label: "Defect type", placeholder: "Short shot, flash, splay, burn, warp...", required: true },
  { key: "scrapQuantity", label: "Scrap quantity", type: "number", placeholder: "45", required: true },
  { key: "notes", label: "Notes", type: "textarea", placeholder: "Inspection details, lot status, containment, or disposition notes" },
  { key: "correctiveAction", label: "Corrective action", type: "textarea", placeholder: "Action taken to contain scrap or prevent recurrence" },
];
...
export default function ScrapPage() {
  ...
  useEffect(() => {
    ...
    if (storedEntries) {
      setEntries(JSON.parse(storedEntries) as ScrapEntry[]);
    }
    setIsLoaded(true);
  }, []);
  ...
  const totalScrapQuantity = useMemo(
    () => entries.reduce((total, entry) => total + toQuantity(entry.scrapQuantity), 0),
    [entries],
  );

  const scrapByDefectType = useMemo(() => {
    const summary = entries.reduce<Record<string, number>>((totals, entry) => {
      const reason = entry.defectType.trim() || "Unspecified";
      totals[reason] = (totals[reason] ?? 0) + toQuantity(entry.scrapQuantity);
      return totals;
    }, {});

    return Object.entries(summary).sort(([, quantityA], [, quantityB]) => quantityB - quantityA);
  }, [entries]);
  ...
  function editEntry(entry: ScrapEntry) {
    setForm({
      date: entry.date,
      shift: entry.shift,
      ...
      <section className="mx-auto max-w-6xl space-y-6">
        <header className="relative overflow-hidden rounded-[2rem] border border-cyan-300/20 bg-slate-900 p-5 shadow-2xl shadow-cyan-950/30 sm:p-8">
          ...
          <div className="relative">
            <p className="text-xs font-bold uppercase tracking-[0.3em] text-cyan-300">Quality tracking</p>
            <h1 className="mt-3 text-3xl font-black tracking-tight text-white sm:text-5xl">Scrap Tracker</h1>
            <p className="mt-3 max-w-3xl text-sm leading-6 text-slate-300 sm:text-base">
              Log scrap entries, corrective actions, and defect-type summaries from a mobile-first dark workspace. Entries are saved in this browser only.
            </p>
          </div>
        </header>

        <section className="grid gap-4 sm:grid-cols-2" aria-label="Scrap totals">
          <article className="rounded-[1.5rem] border border-emerald-300/20 bg-emerald-300/10 p-5 shadow-xl shadow-emerald-950/20">
            <p className="text-4xl font-black text-white">{totalScrapQuantity}</p>
            <h2 className="mt-2 text-sm font-black uppercase tracking-[0.2em] text-emerald-100">Total scrap quantity</h2>
          ...
          <article className="rounded-[1.5rem] border border-cyan-300/20 bg-cyan-300/10 p-5 shadow-xl shadow-cyan-950/20">
            <h2 className="text-sm font-black uppercase tracking-[0.2em] text-cyan-100">Scrap by defect type</h2>
            {scrapByDefectType.length === 0 ? (
              <p className="mt-3 text-sm text-slate-300">No defect summary yet.</p>
            ) : (
              <ul className="mt-4 space-y-3">
                {scrapByDefectType.map(([reason, quantity]) => (
                  <li key={reason} className="flex items-center justify-between gap-4 rounded-2xl bg-slate-950/50 px-4 py-3">
                    ...
                    <span className="shrink-0 text-sm font-black text-cyan-100">{quantity} pcs</span>
                  </li>
                ))}
              </ul>
            )}
        ...
        <form onSubmit={handleSubmit} className="rounded-[2rem] border border-white/10 bg-white/[0.06] p-5 shadow-xl shadow-slate-950/40 sm:p-6">
          <div className="flex items-center justify-between gap-3">
            <h2 className="text-xl font-black text-white">{editingId ? "Edit scrap entry" : "Add scrap entry"}</h2>
          ...
          <button type="submit" className="mt-5 w-full rounded-2xl bg-emerald-300 px-5 py-3 font-black text-slate-950 transition hover:bg-emerald-200 sm:w-auto">
            {editingId ? "Save changes" : "Save scrap entry"}
          </button>
        </form>

        <section className="space-y-4" aria-label="Saved scrap entries">
          ...
          {sortedEntries.length === 0 ? (
            <div className="rounded-[1.5rem] border border-dashed border-cyan-300/30 bg-cyan-300/10 p-6 text-slate-300">
              No scrap entries saved yet. Add the first entry above.
            </div>
          ) : (
              ...
              <article key={entry.id} className="rounded-[1.5rem] border border-white/10 bg-slate-900/90 p-5 shadow-xl shadow-slate-950/30">
                <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                  <div>
                    ...
                    <p className="mt-1 text-sm text-cyan-200">Machine {entry.machineNumber || "—"} · Mold {entry.moldNumber || "—"} · Part {entry.partNumber || "—"}</p>
                  </div>
                ...
                <dl className="mt-5 grid gap-3 sm:grid-cols-2">
                  <div className="rounded-2xl bg-white/[0.05] p-4">
                    <dt className="text-xs font-black uppercase tracking-[0.18em] text-slate-400">Defect type</dt>
                    <dd className="mt-2 whitespace-pre-wrap text-sm leading-6 text-slate-100">{entry.defectType || "—"}</dd>
                  </div>
                  <div className="rounded-2xl bg-white/[0.05] p-4">
                    <dt className="text-xs font-black uppercase tracking-[0.18em] text-slate-400">Scrap quantity</dt>
                    <dd className="mt-2 text-sm font-black leading-6 text-slate-100">{toQuantity(entry.scrapQuantity)} pcs</dd>
                  </div>
                  <div className="rounded-2xl bg-white/[0.05] p-4">
